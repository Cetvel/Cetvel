...
const createUser = catchAsync(async (req: AuthenticatedRequest, res: Response) => {
...
const updateUserPassword = catchAsync(async (req: AuthenticatedRequest, res: Response) => {
    const user = await UserService.updateUserPassword(req.userId!, req.body.currentPassword, req.body.newPassword);
    res.send(user);
})

const updateUserEmail = catchAsync(async (req: AuthenticatedRequest, res: Response) => {
