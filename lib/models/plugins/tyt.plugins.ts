import { ITytDocument,  Section } from "../exam-models/tyt.model";
...
export interface ITytMethods {
    ...
    calculateAnswerStats(objec: ITytDocument): { correctAnswers: number, wrongAnswers: number, empty: number }
}



export const TytMethods = {
    calculateSectionNet: function (this: ITytDocument, section: keyof Section): number {
        const sectionData = this[section];
        return sectionData.correct - (sectionData.wrong * 0.25);
    },
    calculateAnswerStats: function (this: ITytDocument): { correctAnswers: number, wrongAnswers: number, empty: number } {
    ...
    analyzeSectionPerformance: function (this: ITytDocument, section: keyof Section): object {
        const sectionData = this[section];
